  ...
  <div>
    ...
    <el-card class="box-card">
      ...
      <el-row :addDialogVisible="20">
        ...
        <el-col :span="5">
          <el-select class="el-select-search" v-model="selectedMovieArea" placeholder="请选择电影地区" clearable>
            <el-option v-for="item in movieAreaList" :key="item" :label="item" :value="item">
            </el-option>
          </el-select>
        </el-col>
        <el-col :span="4">
      ...
      <el-row :gutter="20" class="row2">
        <el-col :span="10">
          <el-date-picker :unlink-panels="true" v-model="selectedDate" type="datetimerange" range-separator="至"
        ...
        <el-table-column label="操作" width="240">
          <template slot-scope="scope">
            <el-tooltip effect="dark" content="查看电影信息详细" placement="top" :enterable="false" :open-delay="500">
              <el-button type="success" icon="el-icon-view" size="mini" @click="toMovieInfo(scope.row.movieId)">
              </el-button>
            </el-tooltip>
            <el-tooltip effect="dark" content="修改电影信息" placement="top" :enterable="false" :open-delay="500">
              <el-button type="primary" icon="el-icon-edit" size="mini" @click="isAbleEdit(scope.row.movieId)">
              </el-button>
            </el-tooltip>
            <el-tooltip effect="dark" content="删除电影" placement="top" :enterable="false" :open-delay="500">
              <el-button type="danger" icon="el-icon-delete" size="mini" @click="deleteMovie(scope.row.movieId)">
              </el-button>
            </el-tooltip>
            <el-tooltip effect="dark" content="影片类别管理" placement="top" :enterable="false" :open-delay="500">
              <el-button type="warning" icon="el-icon-setting" size="mini"
                @click="showEditCategoryDialog(scope.row.movieId)"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="添加电影" :visible.sync="addDialogVisible" width="60%" @close="addDialogClosed">
      ...
      <el-form :model="addForm" :rules="addFormRules" ref="addFormRef" label-width="100px">
        ...
        <el-form-item label="上映时间" prop="releaseDate">
          <el-date-picker v-model="addForm.releaseDate" value-format="yyyy-MM-dd HH:mm:ss" type="datetime"
            placeholder="选择日期">
          </el-date-picker>
        </el-form-item>
        ...
        <el-form-item label="电影封面">
          <el-upload action="" list-type="picture-card" :auto-upload="false" :limit="1" :file-list="poster"
            :on-exceed="handleExceed" :on-change="handleChangeP" :on-success="handleSuccessP" :on-error="handleError"
            ref="posterRef" :http-request="submitFileP">
        ...
        <el-form-item label="上传视频">
          ...
          <el-upload class="upload-demo" drag action="http://localhost:8181/uploadVideo" :on-success="handleSuccesss"
            multiple>
    ...
    <el-dialog title="修改电影信息" :visible.sync="editDialogVisible" width="60%" @close="editDialogClosed">
      <el-form :model="editForm" :rules="editFormRules" ref="editFormRef" label-width="100px">
        ...
        <el-form-item label="上映时间" prop="releaseDate">
          <el-date-picker v-model="editForm.releaseDate" value-format="yyyy-MM-dd HH:mm:ss" type="datetime"
            placeholder="选择日期">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="电影简介" prop="movieIntroduction">
        ...
        <el-form-item label="电影封面">
          <el-upload action="" list-type="picture-card" :auto-upload="false" :limit="1" :file-list="poster"
            :on-exceed="handleExceed" :on-change="handleChangeP" :on-success="handleSuccessP" :on-error="handleError"
            ref="posterEditRef" :http-request="submitFileP">
            <i slot="default" class="el-icon-plus"></i>
            <div slot="file" slot-scope="{file}">
              <img class="el-upload-list__item-thumbnail" :src="file.url" alt="">
              <span class="el-upload-list__item-actions">
      ...
      <el-form label-width="100px">
        <el-form-item label="电影类型" prop="movieActor">
          <el-select v-model="selectedMovieCategory" placeholder="请选择类型名称" clearable>
            <el-option v-for="item in categoryList" :key="item.movieCategoryId" :label="item.movieCategoryName"
              :value="item.movieCategoryId" v-if="ifShow(item.movieCategoryId)">
            </el-option>
          </el-select>
          <el-button type="primary" @click="addCategory()">添加</el-button>
        </el-form-item>
        <el-form-item>
          <el-tag v-for="tag in editCategoryForm" :key="tag.movieCategoryName" closable
  ...
  export default {
    ...
    methods: {
      handleSuccesss(res, file) {
        ...
        this.addForm.filmUrl = res.data.url
      ...
      async addMovie() {
        ...
        this.$refs.addFormRef.validate(async valid => {
          ...
          if (this.value == 0 || this.value == '免费') {
            this.addForm.isFreeWatch = 0;
          } else if (this.value == 1 || this.value == '收费') {
            this.addForm.isFreeWatch = 1;
          }
          console.log(this.addForm.isFreeWatch)
      ...
      toMovieInfo(id) {
        window.open('http://www.moreluck.cc/movieInfo/' + id)
      },
      isAbleEdit(id) {
        this.checkAbleId.movieId = id
        console.log(this.checkAbleId.movieId)
      ...
      async showEditDialog(id) {
        ...
        await axios.get('sysMovie/find/' + id).then(resp => {
          ...
          if (resp.data.data.isFreeWatch == 0) {
            this.value = '免费'
          } else if (resp.data.data.isFreeWatch == 1) {
            this.value = '收费'
          } else {
            this.value = '其他'
          }
      ...
      async isAbleMultipleDelete() {
        ...
        //   const {
        //     data: res
        //   } = await axios.get('sysSession/isAbleEdit', {
        //     params: this.checkAbleId
        //   })
      ...
      async deleteMovie(id) {
        ...
        const {
          data: res
        } = await axios.post('/sysMovie/delete/' + id)
        if (res.code == 200) {
